...
//!
//! Exercises `any`, `byte`, `byte_range`, `be_u*`, `le_u*`, `take`, `rest`,
...
//!
//! ```sh
//! cargo +nightly fuzz run binary          # continuous fuzzing
//! cargo +nightly fuzz run binary -- -max_len=4096
//! cargo +nightly fuzz run binary corpus/binary  # seed from corpus directory
//! ```
...
use nimble_parsec_rs::typed::{
    any, be_u16, be_u32, be_u64, byte, byte_range, choice, eventually, le_u16, le_u32,
    length_take, rest, satisfy, take, take_while, take_while1, utf8_char, Parser,
};

fuzz_target!(|data: &[u8]| {
    // ── Leaf parsers ──────────────────────────────────────────────────────────

    let _ = any::<&[u8]>().parse_partial(data);
    ...
    // TLV: tag byte (any) + length byte (any) + `length` bytes payload.
    let _ = any::<&[u8]>()
    ...
    let _ = choice([byte::<&[u8]>(0x01), byte(0x02), byte(0x03)]).parse_partial(data);
