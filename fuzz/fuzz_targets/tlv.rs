...
//! arbitrary binary data. Tests the interaction of `byte`, `be_u16`,
...
//! ```sh
//! cargo fuzz run tlv
//! cargo fuzz run tlv corpus/tlv  # seed with hand-crafted corpus
//! ```

#![no_main]

use libfuzzer_sys::fuzz_target;
use nimble_parsec_rs::typed::{be_u16, be_u8, length_take, rest, Parser};
...
fuzz_target!(|data: &[u8]| {
    ...
    //
    // Repeated netstrings with u8 length prefix.

    let netstring = length_take(be_u8::<&[u8]>().map(|n: u8| n as usize));
